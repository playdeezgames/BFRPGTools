Friend Module RaceAbilityRanges
    Friend Function Valid(connection As MySqlConnection, raceId As Integer, abilityId As Integer, abilityScore As Integer) As Boolean
        Using command = connection.CreateCommand
            command.CommandText = $"SELECT COUNT(1) FROM `{Views.RaceAbilityRanges}` WHERE `{Columns.RaceId}`=@{Columns.RaceId} AND `{Columns.AbilityId}`=@{Columns.AbilityId} AND `{MaximumScore}`>=@{Columns.AbilityScore} AND `{MinimumScore}`<=@{Columns.AbilityScore};"
